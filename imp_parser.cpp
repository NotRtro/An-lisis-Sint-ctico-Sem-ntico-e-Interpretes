...
const char* Token::token_names[34] = {
  ...
  // NEW 
  "AND", "OR", "FOR", "COLON", "ENDFOR", "TRUE", "FALSE" };
...
Stm* Parser::parseStatement() {
  ...
  if (match(Token::ID)) {
    ...
    s = new AssignStatement(lex, parseBinaryExp());
  ...
  } else if (match(Token::FOR)) {   // Adding FOR statement
    if (!match(Token::ID)) {
      parserError("Esperaba un 'ID'");
    }
    if (!match(Token::COLON)) {
      parserError("Esperaba un ':'");
    }
    e = parseBinaryExp();
    if (!match(Token::COMMA)) {
      parserError("Esperaba una ','");
    }
    ee = parseBinaryExp();
    if (!match(Token::DO)) {
      parserError("Esperaba 'do'");
...
Exp* Parser::parseBinaryExp() {
  Exp *e, *rhs;
  e = parseComparisonExp();
...
Exp* Parser::parseFactor() {
  ...
  if (match(Token::CONDEXP)) {
    ...
    Exp* c = parseBinaryExp();
    if (!match(Token::COMMA)) parserError("Expecting comma");
    Exp* et = parseBinaryExp();
    if (!match(Token::COMMA)) parserError("Expecting comma");
    Exp* ef = parseBinaryExp();
